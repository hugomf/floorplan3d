...
class WallModel {
  ...
  void resizeWall(Offset newPosition) {
    ...
    if (isResizingLeftEdge) {
      ...
      wall.width -= deltaX; // Reduce the width
      wall.position = Offset(wall.position.dx + deltaX, wall.position.dy); // Move the position
    } else {
      // Resize from the right edge
      wall.width += deltaX; // Increase the width
    }
        ...
        child: GestureDetector(
          ...
          child: Listener(
            onPointerHover: (event) {
              ...
              if (selectedIndex != null) {
                final wall = _wallModel.walls[selectedIndex];
                if (_isNearLeftEdge(position, wall) || _isNearRightEdge(position, wall)) {
                  setState(() {
                    _cursor = SystemMouseCursors.resizeLeftRight; // Change cursor to resize icon
                  });
                } else {
                  setState(() {
...
class WallPainter extends CustomPainter {
  ...
  void _drawWalls(Canvas canvas) {
    ...
    final List<Path> wallPaths = walls.map((wall) => Path()
      ..addRect(Rect.fromLTWH(wall.position.dx, wall.position.dy, wall.width, Wall.height)))
      .toList();
...
void _drawMeasurementGuides(Canvas canvas, Rect rect) {
  ...
  _drawMeasurementGuideWithArrows(canvas, rect.left, rect.right, topGuideY, arrowSize, rect.width, textPadding);

  // Draw the bottom measurement guide
  final bottomGuideY = rect.bottom + guideHeight;
  _drawMeasurementGuideWithArrows(canvas, rect.left, rect.right, bottomGuideY, arrowSize, rect.width, textPadding);
}


/// Draws a measurement guide with arrows, vertical lines, and text.
void _drawMeasurementGuideWithArrows(Canvas canvas, double startX, double endX, double y, double arrowSize, double width, double textPadding) {
  ...
  bool _doWallsOverlap(Wall wall1, Wall wall2) {
    return Rect.fromLTWH(wall1.position.dx, wall1.position.dy, wall1.width, Wall.height)
        .overlaps(Rect.fromLTWH(wall2.position.dx, wall2.position.dy, wall2.width, Wall.height));
  }
